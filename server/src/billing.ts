 ...
 */
export async function createSubscription(userId: string, price: string, payment_method: string) {
...
export async function cancelSubscription(userId: string, subscriptionId: string) {
  ...
  if (subscription.status === 'canceled') {
    await db
      ...
      .doc(userId)
      .update({
        activePlans: firestore.FieldValue.arrayRemove(subscription.items.data[0].price.id)
      })
  }
